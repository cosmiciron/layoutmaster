...
const protectedTargetFiles = new Set([
    'core/index.ts'
]);
...
function main() {
    ...
    const targetFiles = walkFiles(targetRoot).map(normalize);
    const extraFiles = targetFiles.filter((relativePath) =>
        !sourceFileSet.has(relativePath)
        && !protectedTargetFiles.has(relativePath)
    ...
    console.log(`[sync-shared-engine] files=${sourceFiles.length} changed=${changed} missing=${missing} extra=${extraFiles.length}`);

    if (checkOnly) {
        if (changed > 0 || extraFiles.length > 0) {
    ...
    console.log(`[sync-shared-engine] copied=${copied} pruned=${pruned}`);
}
